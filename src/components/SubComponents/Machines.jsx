...
import { PhotoProvider, PhotoView, I } from "react-photo-view";
...
const Machines = () => {
  ...
  const images = [
    "/images/slotmachines-3.png",
    "/images/slotmachines-2.png",
    "/images/supergameMachine.png",
    "/images/supergameMachineVideo.mp4",
  ];
  return (
    <Stack flex={1} spacing={{ base: 5, md: 10 }}>
      ...
      <SimpleGrid columns={columns} spacing={4} position="relative">
        {images.map((item, index) => (
          <PhotoProvider key={index} src={item}>
            {item?.endsWith(".mp4") ? "" : <img src={item} alt="" />}
          </PhotoProvider>
        ))}
        ...
        <>
          {images.map((item, index) =>
            item?.endsWith(".mp4") ? (
              <Box fit={"contain"} align={"center"} height="27rem">
                <video
                  alt={"Machine Image"}
                  controls
